        ...
        switch (true) {
            ...
            case /^https:\/\/api\.zhihu\.com\/people\/self/.test(magicJS.request.url):
                ...
                try {
                    ...
                    magicJS.logNotify(`用户登录用户信息，接口响应：${magicJS.response.body}`);
                    if (obj && obj['id'] && obj.hasOwnProperty('vip_info') && obj['vip_info'].hasOwnProperty('is_vip')) {
                        ...
                        magicJS.logNotify(`当前用户id：${obj['id']}，是否为VIP：${obj['vip_info']['is_vip']}`);
                    }
                    else {
            ...
            case /^https:\/\/api\.zhihu\.com\/topstory\/recommend\?/.test(magicJS.request.url):
                try {
                    ...
                    magicJS.logError(`知乎推荐列表去广告异常：${err}`);
                }
                break;
            // 关注列表去广告
            case /^https?:\/\/api\.zhihu\.com\/moments(\/|\?)?(recommend|action=|feed_type=)(?!\/people)/.test(magicJS.request.url):
                try {
                    ...
                    magicJS.logError(`知乎关注列表去广告异常：${err}`);
                }
                break;
            // 回答列表去广告及黑名单增强
            case /^https?:\/\/api\.zhihu\.com\/v4\/questions/.test(magicJS.request.url):
                try {
                    ...
                    custom_blocked_users = !!custom_blocked_users ? custom_blocked_users : {};
                    let obj = JSON.parse(magicJS.response.body);
                    magicJS.logNotify(`当前黑名单列表: ${JSON.stringify(custom_blocked_users)}`);
                    delete obj['ad_info'];
                    ...
                    magicJS.logError(`知乎回答列表去广告异常：${err}`);
                }
                break;
            // 拦截官方账号推广消息
            case /^https?:\/\/api\.zhihu\.com\/notifications\/v3\/timeline\/entry\/system_message/.test(magicJS.request.url):
                try {
                    ...
                    magicJS.logError(`知乎拦截官方账号推广消息异常：${err}`);
                }
                break;
            // 屏蔽官方营销消息
            case /^https?:\/\/api\.zhihu\.com\/notifications\/v3\/message\?/.test(magicJS.request.url):
                try {
                    ...
                    magicJS.logError(`知乎屏蔽官方营销消息异常：${err}`);
                }
                break;
            // 黑名单管理
            case /^https?:\/\/api\.zhihu\.com\/settings\/blocked_users/.test(magicJS.request.url):
                ...
                magicJS.logNotify(`当前用户id：${userInfo.id}，脚本黑名单：${JSON.stringify(custom_blocked_users)}`);
                // 获取黑名单
                if (magicJS.request.method == 'GET') {
                    try {
                        let obj = JSON.parse(magicJS.response.body);
                        if (!!obj['data']) {
                            magicJS.logNotify(`本次滑动获取的黑名单信息：${JSON.stringify(obj['data'])}`);
                            ...
                            if (obj['paging']['is_end'] == true) {
                                ...
                                magicJS.logNotify(`脚本黑名单内容：${JSON.stringify(custom_blocked_users)}。`);
                            }
                        }
                        else {
                ...
                // 写入黑名单
                    ...
                    try {
                        ...
                        if (obj.hasOwnProperty('name') && obj.hasOwnProperty('id')) {
                            magicJS.logNotify(`当前需要加入黑名单的用户Id：${obj['id']}，用户名：${obj['name']}`);
                            custom_blocked_users[obj['name']] = obj['id'];
                            magicJS.write(blocked_users_key, custom_blocked_users, userInfo.id);
                            magicJS.logNotify(`${obj['name']}写入脚本黑名单成功，当前脚本黑名单数据：${JSON.stringify(custom_blocked_users)}`);
                ...
                // 移出黑名单
                    ...
                    try {
                        ...
                        if (obj.success) {
                            ...
                            magicJS.logNotify(`当前需要移出黑名单的用户Id：${user_id}`);
                            for (let username in custom_blocked_users) {
                                if (custom_blocked_users[username] == user_id) {
                                    delete custom_blocked_users[username];
                                    magicJS.write(blocked_users_key, custom_blocked_users, userInfo.id)
                                    magicJS.logNotify(`${username}移出脚本黑名单成功，当前脚本黑名单数据：${JSON.stringify(custom_blocked_users)}`);
            ...
            case /^https?:\/\/api\.zhihu\.com\/search\/preset_words\?/.test(magicJS.request.url):
                try {
                    if (!!magicJS.response.body) {
                        magicJS.logNotify(`预置关键字返回：${magicJS.response.body}`);
                        let obj = JSON.parse(magicJS.response.body);
